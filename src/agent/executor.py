...
{{
    ...
    "dashboard": {{
        ...
        "skill_assessment": {{
            "<activated_skill_id>": {{
                "joint_assessment": "仅基于该策略自身规则与依赖输入的结论",
                "signal": "strong_buy|buy|hold|sell|strong_sell",
                "confidence": 0.0,
                "decisive_evidence": [{{"tool": "该策略声明的工具名", "fields": ["实际返回字段"], "summary": "字段如何支持结论"}}],
                "conditions_met": ["已满足条件"],
                "conditions_missed": ["未满足条件"],
                "limitations": ["缺失或降级数据限制"]
            }}
        }}
    }},
...
- 激活一个或多个交易技能时，必须输出 `dashboard.skill_assessment`；以每个激活技能的精确 ID 为键逐项输出对象，不得遗漏、合并或复制总体结论。每项必须包含 `joint_assessment`、五态 `signal`、0-1 `confidence`、结构化 `decisive_evidence`、`conditions_met`、`conditions_missed`、`limitations`。`decisive_evidence.tool` 只能引用该策略声明的依赖工具，`fields` 只能填写该工具本次真实返回且非空的字段；没有数据时写入 limitations/conditions_missed，不得补造字段或结论。完成各策略独立评估后，才可汇总总体结论。
...
- 只要激活了交易技能，最终回答必须单独列出“策略分析详情”；多选时按激活技能的精确 ID 逐项展示，禁止合并、遗漏或用综合结论代替单个策略结论。
- 每个策略都必须完整展示：判定信号、判定依据、满足条件、未满足条件、依赖数据及其实际值/来源、数据限制。条件列表为空时也要明确写“无”，不得只展示满足条件。
- 每个策略必须使用独立小节，并按“依赖数据 → 条件核对 → 策略结论”的顺序输出。依赖数据逐项列出指标名、实际值、单位、数据时间、来源和可用状态；条件核对逐项列出策略原始规则/阈值或价格边界、比较所用实际值以及“满足/未满足/无法判断”，不得只写概括性分析。
- 策略声明的每项依赖数据都必须展示。只能引用本次工具真实返回的值；字段缺失、工具失败、降级、过期或未调用时，保留该数据项并明确标为“缺失/失败/降级/过期/未获取”及原因，禁止省略、补造或借用其他策略的数据。
- 缺少必需数据时，该策略必须显示“证据不足/无法完整评估”，并在“未满足条件”和“数据限制”中列出缺口；不得把缺失数据当成条件已满足，也不得让无依据结论参与多策略综合。
- 全部策略逐项展示完毕后才可给出多策略综合结论；展示口径与首页分析报告的策略分析详情保持一致。
...
- 用户选择了一个或多个交易技能时，先从已保存分析上下文读取对应策略的证据；最终回答必须单独列出“策略分析详情”，并按所选技能的精确 ID 逐项展示，禁止合并、遗漏或用综合结论代替单个策略结论。
- 每个策略都必须完整展示：判定信号、判定依据、满足条件、未满足条件、依赖数据及其实际值/来源、数据限制。条件列表为空时也要明确写“无”，不得只展示满足条件。
- 每个策略必须使用独立小节，并按“依赖数据 → 条件核对 → 策略结论”的顺序输出。依赖数据逐项列出指标名、实际值、单位、数据时间、来源和可用状态；条件核对逐项列出已保存的策略规则/阈值或价格边界、比较所用实际值以及“满足/未满足/无法判断”，不得只写概括性分析；上下文未保存规则边界时明确写“规则边界未保存”。
- 所选策略声明的每项依赖数据都必须展示。已保存上下文缺少字段、来源或整个策略记录时，仍保留该策略和数据项并明确标为“缺失/未保存/无法获取”，禁止省略、补造或借用其他策略的数据。
- 缺少必需数据时，该策略必须显示“证据不足/无法完整评估”，并在“未满足条件”和“数据限制”中列出缺口；全部策略逐项展示完毕后才可给出多策略综合结论。
- 展示口径与首页分析报告的策略分析详情保持一致，并明确说明结论基于已保存数据。
