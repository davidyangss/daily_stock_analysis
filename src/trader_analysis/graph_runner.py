        ...
        class DsaTradingAgentsGraph(base_factory):  # type: ignore[misc, valid-type]
            def resolve_instrument_context(self, ticker: str, asset_type: str = "stock") -> str:
                return (
                    ...
                    "将某日期归因于密集成交区或套牢筹码带前，必须核对该日 low-high 区间与所述价格带确实相交；"
                    "若该日最高价仍低于价格带下沿，只能描述为跌破后的压力确认，不得算作该价格带的成交重叠来源。"
                    "阻力层级必须按价格由低到高整理；当均线或其他单点阻力位于某个筹码带内部时，"
                    "应写成该筹码带内的共振压力，不得重复描述成两个彼此独立的连续阻力层。"
                    "基本面报告必须逐项保留数据性质与报告期：业绩预告不得标成正式季报，半年度预告应标明 H1/06-30，"
                    "一季报应标明 Q1/03-31；不得把统一的 report_date 套到不同期间的字段。"
                    "营收、净利润、经营现金流等指标只有在报告期和数据性质一致时才可计算利润率、现金含量或相互比较；"
                    "不同期间的数据必须分开展示并明确说明不可直接相除。"
                    "同时存在定期报告和业绩预告时必须输出两张独立表：定期报告表列示同期间营收、归母净利润、"
                    "经营现金流及可验证的同期间比率；业绩预告表必须标明预告值或区间/中值口径，未披露现金流时明确写未披露，"
                    "并说明预告结论需等待正式报告验证。盈利质量判断只能依据同期间数据，禁止因混期计算得出质量差等结论。"
