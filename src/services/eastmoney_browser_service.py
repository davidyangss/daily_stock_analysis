...
class PlaywrightBrowserAdapter(BrowserAdapter):
    ...
    def launch(self, profile_dir: str, executable_path: str, headless: bool) -> None:
        ...
        self._context = self._pw.chromium.launch_persistent_context(
            ...
            executable_path=executable_path or _CHROME_EXECUTABLE,
            headless=headless,
            args=["--no-first-run", "--no-default-browser-check"],
        )
        pages = self._context.pages
        self._page = pages[0] if pages else self._context.new_page()
        logger.info("[PlaywrightBrowserAdapter] 浏览器已启动，profile=%s", profile_dir)
    ...
    def check_login(self) -> bool:
        ...
        try:
            ...
            logged_in = bool(uidal) or (bool(ut) and len(ut) > 20) or bool(empinf)
            logger.debug(
                "[PlaywrightBrowserAdapter] check_login: em_cookies=%d uidal=%s ut_len=%d → %s",
                len(em), bool(uidal), len(ut), logged_in,
            )
            return logged_in
        except Exception as exc:
            logger.warning("[PlaywrightBrowserAdapter] check_login 异常: %s", exc)
    ...
    def fetch_kline(self, secid: str, lmt: int, timeout_seconds: int) -> Dict[str, Any]:
        """
        用 page.goto() 直接导航至东财 K 线 API URL。
        走 Chrome 真实网络栈（TLS/HTTP2 指纹与 Cookie 完整），和地址栏访问等价。
        """
        import json as _json
        if not _SECID_RE.match(secid):
            raise ValueError(f"secid 格式非法: {secid!r}")
        ...
        # wait_until="commit" 拿到响应头即可，不等页面渲染
        resp = self._page.goto(url, timeout=timeout_seconds * 1_000, wait_until="commit")
        if resp is None:
            raise RuntimeError("page.goto 无响应")
        text = resp.text()
        return _json.loads(text)
    ...
    def close(self) -> None:
        if self._context is not None:
            try:
                ...
                logger.warning("[PlaywrightBrowserAdapter] context.close: %s", exc)
            self._context = None
            self._page = None
        if self._pw is not None:
            try:
                ...
                logger.warning("[PlaywrightBrowserAdapter] playwright.stop: %s", exc)
            self._pw = None
