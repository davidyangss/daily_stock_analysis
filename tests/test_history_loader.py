...
def _valid_cached_bar(bar_date: str) -> dict:
    return {
        "date": bar_date,
        "open": 10,
        "high": 11,
...
class HistoryLoaderTestCase(unittest.TestCase):
    ...
    def test_returns_db_data_when_sufficient(self, mock_get_db):
        ...
        fake_bar = MagicMock()
        fake_bar.to_dict.return_value = _valid_cached_bar("2026-04-17")
        mock_db = MagicMock()
    ...
    def test_uses_frozen_target_date_from_contextvar(self, mock_get_db):
        ...
        try:
            mock_db = MagicMock()
            fake_bar = MagicMock()
            fake_bar.to_dict.return_value = _valid_cached_bar("2026-04-15")
    ...
    @patch("src.services.history_loader._get_fetcher_manager")
    @patch("src.storage.get_db")
    def test_incomplete_local_ohlcv_falls_back_to_provider(self, mock_get_db, mock_get_fm):
        from src.services.history_loader import load_history_df

        incomplete_bar = MagicMock()
        incomplete_bar.to_dict.return_value = {
            "date": "2026-04-17",
            "close": 10.5,
            "data_source": "TencentFetcher",
        }
        mock_get_db.return_value.get_data_range.return_value = [incomplete_bar] * 30
        remote_df = pd.DataFrame({"close": [10.5]})
        mock_get_fm.return_value.get_daily_data.return_value = (remote_df, "TencentFetcher")

        _df, source = load_history_df("600519", days=30, target_date=date(2026, 4, 18))

        self.assertEqual(source, "TencentFetcher")
        mock_get_fm.return_value.get_daily_data.assert_called_once_with("600519", days=30)

    @patch("src.services.history_loader._get_fetcher_manager")
    @patch("src.storage.get_db")
    def test_mixed_local_adjustments_fall_back_to_provider(self, mock_get_db, mock_get_fm):
        from src.services.history_loader import load_history_df

        bars = []
        for index in range(30):
            bar = MagicMock()
            payload = _valid_cached_bar("2026-04-17")
            payload["data_source"] = "TencentFetcher" if index < 29 else "TushareFetcher"
            bar.to_dict.return_value = payload
            bars.append(bar)
        mock_get_db.return_value.get_data_range.return_value = bars
        remote_df = pd.DataFrame({"close": [10.5]})
        mock_get_fm.return_value.get_daily_data.return_value = (remote_df, "TencentFetcher")
