...
const fieldTitleMap: Record<string, string> = {
  ...
  PROVIDER_LOOP_TIMEOUT_SECONDS: '默认单数据源超时',
  PROVIDER_LOOP_TOTAL_TIMEOUT_SECONDS: '数据源循环总超时',
  PROVIDER_TIMEOUT_OVERRIDES: '数据源独立超时',
  ...
  AGENT_SKILL_AGENT_TIMEOUT_S: '策略专家超时（秒）',
  AGENT_STOCK_INFO_TIMEOUT_SECONDS: '策略基本信息总超时（秒）',
  AGENT_STOCK_INFO_PROVIDER_TIMEOUT_SECONDS: '策略基本信息单源超时（秒）',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  AGENT_SKILL_AGENT_TIMEOUT_S: "普通策略 Specialist 的独立超时上限；实际取本值与编排剩余预算的较小值，0 表示关闭独立上限。",
  AGENT_STOCK_INFO_TIMEOUT_SECONDS: "普通 Agent get_stock_info 的总预算；不改变交易员分析或共享 Provider 的全局超时。",
  AGENT_STOCK_INFO_PROVIDER_TIMEOUT_SECONDS: "普通 Agent get_stock_info 内估值补充、所属板块等单来源预算。",
