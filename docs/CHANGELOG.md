...
- [修复] 问股历史会话在重新加载后仍展示每条 AI 回复实际使用的 LLM，不再因技能与后端临时元数据缺失而隐藏模型徽标
- [改进] 问股会话中的“策略证据 / 策略分析详情”整体默认折叠，展开后逐策略“判定依据”连同满足/未满足条件表及综合“判定依据”仍分别默认折叠
- [改进] 首页报告在概览区标记实际分析模型，AI 问股随消息保存并展示本轮实际使用的单个或多个 LLM，模型元数据仅用于展示且不参与运行时路由
- [修复] 问股单选与多选策略在模型正文后确定性追加首页同源的逐策略证据，完整展示依赖数据、规则阈值/价格边界、实际比较值、满足与未满足条件及缺失/失败/降级状态，便于用户自评；证据不足时保留策略与数据缺口并禁止跨策略借用
- [修复] 首页移动端历史抽屉固定在全局标题栏下方，避免标题栏遮住“历史 / 自选 / 今日”标签
- [改进] 首页历史、自选与今日列表改为每页 5 条并将分页栏吸附在列表底部；历史按每次分析逐条展开且不再按股票合并，历史与今日按分析时间倒序排列，列表仅展示情绪分数与可换行的分析策略名称、不显示策略前缀和操作结论，移动端标签栏固定在列表上方，并在报告详情头部显示记录 ID
- [修复] 首页与交易员分析的股票自动补全在选择标的时仅回填输入框，统一等待用户点击“分析”或“开始分析”后再创建任务，避免误触发分析
...
- [修复] 普通策略 Specialist 默认增加 120 秒独立上限，`get_stock_info` 使用 60/8 秒分层调用预算并显式披露缺失；StrategyEngine 的 consensus 汇总不再伪装成额外已选策略
- [修复] 普通策略分析在 `standard/full` 模式对已选策略执行隔离 Specialist，再由 StrategyEngine 确定性汇总；联合兼容路径要求逐策略结构化依据，禁止跨策略借用证据或补造缺失字段
- [改进] 共享日线与新闻 Provider 仅在本地数据通过时效、完整性、口径和直接相关性准入时优先复用，否则记录原因并继续远端 fallback，不改变交易员分析独立工作流
- [改进] AI 问股复用逐策略隔离与汇总路径，AI 建议把策略输入质量纳入确定性护栏，策略证据不足时将可执行买入/加仓降级为观察
- [修复] 普通策略复用 Pipeline 已按优先级逐源收敛的基本面输入，避免 Specialist 对同一数据再次获取；单来源失败只降级该输入，不阻断其他策略、综合决策或完整报告
- [修复] `get_stock_info` 的估值与成长指标全部齐全时，不再因可选基本面子域为 `partial` 而误报策略必需数据降级；真实缺失字段仍保留 `partial` 与数据限制
- [修复] Web 与独立 Markdown 报告把每种策略的输出、满足/未满足条件表和“策略分析输入数据”表合并展示；数据限制表明确列出仍需补充的指标、当前来源及失败原因，并将概念板块排名与数据降级提示中文化；首页相关资讯默认折叠且标题吸顶
- [改进] 首页策略分析的“策略分析输出”和“策略分析输入数据”支持默认折叠、标题行折叠图标与展开后在首页滚动容器顶部吸顶，兼容 PC 与移动端
...
- [新功能] 交易员报告新增完整数据证据清单，保存全部标准化行情、快照、基本面、新闻与社区输入，并区分预检已加载和工具实际消费
- [修复] 交易员历史基本面按公告日或可得日执行 point-in-time 准入，清除无法回溯的运行时估值、资金、机构、龙虎榜和板块数据，且 optional 基本面缺失不再阻断核心行情分析
- [修复] 交易员日线向多数据源 fallback 传递建议历史深度，早期来源数据过浅时继续后续来源，全部不足时仍保留最大 partial 供新股降级判断
- [修复] 交易员新闻与社区工具统一日期窗口，社区固定检索近 7 天，并明确区分检索 provider、内容 publisher、来源域名、发布时间与采集时间
- [改进] TradingAgents 情绪注入新增可选 provider-neutral sections，国内证据使用真实来源标签并与 system instruction 分离，未注入时保留原版美股情绪路径
- [修复] 交易员全市场新闻在缺少独立 A 股宏观证据时明确不可用，三类财务工具只返回各自报表字段，避免个股新闻和整包基本面被错误复用
- [修复] 交易员 decision memory 在缺少同口径国内 benchmark 时保持 pending，避免 `alpha_return=None` 进入上游百分比格式化路径
- [改进] 交易员全局身份 Prompt 收敛为证券身份、简体中文、A 股术语、人民币和证据边界，本地技术公式与时点约束下沉到确定性工具和数据契约
- [修复] 问财基本面保留字段级报告期，并在交易员证据进入模型前拆分单一来源内部混合的季度与半年度财务数据
- [修复] 交易员市场分析校验套牢筹码带与归属交易日的实际价格交集，区分成交重叠来源和跌破确认，并合并带内均线共振阻力
- [修复] 多来源基本面禁止跨报告期补齐同一财报对象，并要求交易员分析分开展示业绩预告与正式财报、避免混期比率
- [修复] 交易员分析市场技术指标统一使用 MACD 为 DIF、MACD 柱为 2 倍 DIF 与 DEA 之差，并将 RSI(14) 改为 Wilder EMA/SMMA 口径
- [修复] 交易员市场分析按日线 low 精确引用区间最低价及日期，区分价格倍数与累计涨幅，并禁止在证据未标明时臆断前复权口径
- [修复] 交易员分析布林带统一使用 20 日总体标准差，并要求报告中的指标值与所述交易日严格对应
- [修复] 交易员市场分析按 OHLC 实际计算 K 线实体与上下影线，将开盘价等于最高价且收盘显著走低的形态描述为高开长阴而非长上影
- [修复] 交易员分析取消“分析日期早于当天即禁用基本面”的拦截，改为读取最近一期财报、展示报告期，并仅在财报超过一年时判为不可用
- [改进] 交易员分析的数据质量问题、能力状态、数据源和数据时间改用中文展示
- [改进] PC 版交易员分析报告的模块标签栏支持滚动吸顶，长报告切换模块时无需返回顶部
- [修复] 交易员分析完整报告的模块标签栏在飞书 Web 窄屏容器中同样保持吸顶，避免滚动长报告时丢失模块切换入口
- [改进] 首页股票分析的输入数据块在折叠状态直接列出受限数据项、可用性状态及缺失或失败原因，避免仅显示“部分可用”的汇总提示
- [改进] 首页策略证据的必需输入限制行会展示基本面实际缺失字段、数据源限制及 fallback 失败原因，不再仅显示“股票基础资料与基本面”总类
- [修复] 首页移动端打开策略下拉菜单时提升其父层叠层级，避免被策略分析的输入或输出吸顶标题遮挡
- [修复] 交易员分析在 A 股本地名称索引未命中时通过已配置的问财接口补全股票名称，再开始生成报告
- [改进] 问股页面取消单次最多选择 3 个策略的前端限制，可按需组合全部可用策略
- [修复] 多数据源基本面改为按 provider 逐项补缺并保留先前有效结果，避免后续来源超时清空营收增长、ROE、毛利率等已获取字段
- [改进] 数据能力统一采用可配置 provider 顺序、单来源超时和总预算，并记录禁用、不可用与未实现来源的跳过诊断
- [改进] Provider 循环支持按数据源分别配置超时时间，未覆盖来源默认等待 60 秒
- [修复] 基本面 Provider 已返回失败或不支持时立即切换下一来源，并实时记录每次调用的开始、完成、空结果与超时状态
- [修复] 日线缓存命中和分析历史复用路径补充数据源诊断，避免有效日线被误报为未记录诊断信息
- [改进] Web 首次访问默认使用亮色主题，同时保留用户已保存的主题选择
- [改进] 交易员情绪分析改为通过 SearXNG 独立检索雪球、知乎和微博社区观点，不再复用新闻证据
- [改进] 交易员新闻与情绪报告追加证据摘要、来源、内容时间、采集时间和原文链接，便于交叉核验
- [修复] 交易员情绪提示明确标识 DSA 社区搜索摘要的真实来源与非正文属性，禁止模型声称已自行联网或读取未提供内容
- [新功能] 交易员新闻与情绪证据支持可选的 agent-browser 受控公开页面正文摘录，通过 HTTPS、域名/DNS 校验、页数、超时和字符数护栏限制采集，失败时保留搜索摘要降级继续

- [修复] 首页“追问 AI”进入问股页时新建独立对话，避免误选并复用已有历史会话

- [新功能] 交易员分析角色模型调用纳入 Token 用量统计，并在 Web 用量页面按交易员分析类型展示
- [修复] Web 标题栏以菜单按钮文本色稳定居中显示当前选中的菜单标题，并兼容带尾部斜杠的页面路径
- [修复] Web 手机版问股输入框随换行提示和内容自动增高，并修复首页个股栏在移动浏览器中无法触摸滚动
- [修复] Web 手机版问股标题操作栏固定于全局标题栏下方，窄屏使用单行图标操作避免遮挡说明，并修复历史会话抽屉被覆盖且无法触摸滚动的问题；PC 端导出会话与发送按钮垂直居中
- [修复] Web 手机版修复 AI 建议信号详情标题遮挡、持仓日期控件重叠、交易员分析折叠卡片底部留白，以及告警创建表单越界且无法横向滑动的问题
- [修复] Web 导航菜单中的界面语言操作不再错误显示为当前选中菜单项
- [改进] Web 桌面端与手机版统一使用固定顶部标题栏，菜单默认收起并按设备显示文字或图标
- [修复] Web 手机版使用完整动态视口，隔离首页个股栏与底层内容滚动，并避免窄屏表格单元格重叠
- [修复] Web 移动端页面内容宽度与顶部菜单区域对齐，移除遮挡页面标签的顶部语言及主题悬浮入口，同时保持移动端侧栏菜单左对齐、桌面端侧栏菜单居中
- [修复] 交易员分析任务选中后联动更新分析表单中的股票代码与分析日期
- [改进] 交易员分析移除与完整运行流重复的“运行状态 / 角色流程”区域
- [改进] 交易员分析取消运行任务前增加确认警示，将数据质量与完整报告按序置于运行流前并默认折叠，提高数据质量标题对比度，并使四类详情标题在展开滚动时吸附于页面标题栏下方
- [修复] 交易员分析补齐运行时基本面快照、部分可用等质量问题的中文显示映射，内部 API/Trace 继续保留英文稳定代码
- [修复] 交易员分析要求市场技术分析从首句开始使用简体中文，并在正式报告层清理模型泄漏的英文前置分析草稿，同时保留原始 LLM Trace
- [改进] 交易员分析正式角色报告使用中文主显示，并为 Rating、Underweight、Overall Sentiment、MACD 等固定字段、枚举或技术缩写保留必要英文对照
- [改进] 交易员分析的 Debug 日志默认折叠并在展开时加载；选中任务后立即加载运行轨迹，使角色流程与当前任务一致
- [改进] 交易员分析将刷新和真实取消操作下沉到各任务列表项，并新增与策略分析一致的完整流程图，展示预检、并行角色分析、研究/风险决策及报告输出状态
- [改进] 交易员分析运行流默认折叠，仅在展开后获取轨迹并渲染完整流程图，角色摘要状态独立随任务更新
- [修复] 交易员分析阻止内部详情折叠事件触发外层日志面板重新加载，并将完整报告改为可浮动快速切换的多标签展示
- [修复] 交易员分析浮动报告标签栏滚动吸附时贴合页面顶部，不再保留多余间距

- [新功能] 交易员分析增加完整中文角色报告展示及合并 Markdown 下载

- [改进] 交易员分析按单次 LLM 与工具步骤展示脱敏后的详细输入、输出、耗时和错误，完成后合并重复的开始记录，并从交互列表排除普通生命周期事件
- [修复] AkShare 东方财富日线调用使用有界子进程超时，交易员分析总超时后可靠进入失败终态，避免阻塞预检时长期显示运行中
- [修复] 交易员分析对新上市标的停止无效的全数据源补历史轮询，至少 3 个有效交易日时以短历史警告降级继续，并为每项预检证据记录可见的输入输出事件
- [修复] 历史交易员分析在日线与核验价格可用且无阻断问题时允许以 degraded 生成完整角色报告，同时保留新闻、财务和情绪的 point-in-time 缺失限制
- [改进] 交易员分析 Graph 异常在任务错误详情中保留安全错误消息并记录服务端 traceback，避免只显示无法定位的异常类型
- [修复] 交易员分析将带自定义 Base URL 的 OpenAI deployment 作为 OpenAI-compatible 网关注入 TradingAgents，正确传递 deployment 密钥并避免误用原生 OpenAI 环境变量与 Responses API
- [修复] 交易员分析在 TradingAgents 配置复制完成后注入运行级取消回调，避免线程锁无法 deepcopy 导致 Graph 初始化失败

- [改进] 交易员分析任务列表展示唯一任务 ID，改为每页 5 条并新增翻页；报告、Debug 日志和脱敏 LLM 交互以任务 ID 关联持久化到 SQLite 并在 Web 展示

- [新功能] 交易员分析新增持久化任务列表，可在服务重启后查询全部运行，并在 Web 中追踪任务阶段、角色进度、关联报告及脱敏的 LLM/工具交互

- [新功能] 新增独立 SQLite 历史行情库，日线按市场、代码、复权类型和日期存储，历史请求优先读库、缺失时按来源优先级抓取并入库，同时记录供应商纠错修订且不固化今日临时数据

- [修复] 交易员分析正确解包日线数据及来源，日线来源记录不足时继续 fallback，并跳过少量异常历史 K 线后降级提示，避免已有足够有效数据时误判为空或停止分析；Web 任务状态、角色、事件与错误提示优先显示中文
...
- [修复] 问财基本面适配器独立识别业绩快报并归一化多行十大股东持股变化，基本面聚合在报告所需财务、业绩和治理字段齐全前继续按优先级补缺
...
- [改进] 完整 Markdown 报告将摘要、趋势、量能和筹码等并列数据改为带明确列名的表格；筹码指标补充含义并规范百分比与小数精度，避免展示无标签原始浮点数。
- [改进] 策略关键数据证据在完整 Markdown 与 WebUI 中统一使用“指标、状态、数值、含义”表格，并按数据概览、关键指标和数据限制分层展示。
- [改进] WebUI 与完整 Markdown 的每项策略数据证据改为两层结构，数据来源等元信息在首行展示，关键值表格在下一行独占完整宽度。
- [改进] 策略证据中的元、股等数值展示增加千分位分隔，旧历史报告也在 Web 展示时兼容格式化；将“一根日K线”改为“一个交易日”的用户文案，同时保持原始数值字段不变。
...
- [改进] A 股筹码分布保留东财成品数据优先级，并在东财异常、超时、空数据或字段无效时依次使用新浪、腾讯不复权日线与换手率本地计算，避免单一平台故障导致筹码能力整体不可用。
- [修复] 统一将主力资金流向和综合情报搜索工具名以中文展示，避免用户界面泄露内部函数名。
...
- [修复] 策略必需数据未调用或缺失时将对应观点排除出多策略投票，并在数据质量限制中保留 missing、not_supported、partial 与 estimated 状态，避免静默成功。
- [新功能] 个股报告、历史详情、Web 与通知新增策略关键数据证据清单，展示必需工具状态、关键值、来源、时效/记录覆盖和缺失原因。
- [改进] 股票名称解析复用完整股票索引，支持企业全称、英文名、拼音与别名，并对跨市场同名企业返回可选择候选，避免公司名被误判为股票代码。
- [新功能] 问股支持从自然语言中的中文股票全称、短名和别名解析唯一标的；多候选时不再猜测市场，改为展示可点击候选并要求用户选择后再分析。
- [改进] 策略关键数据证据以中文说明工具用途和获取内容；成功时展示实际数据源及其公开链接，失败时展示数据源、请求操作和脱敏原因。
