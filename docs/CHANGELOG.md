...
- [修复] SerpAPI 新闻搜索支持 `SERPAPI_TIMEOUT_SECONDS` 配置（默认 20 秒），并使用不可被该配置放大的 20 秒硬截止与有界阻塞槽位，避免 SDK 未响应时长期占用分析任务线程
- [修复] 个股分析生成每日大盘上下文时，在上层编排共享 48 秒行情预算和 60 秒完整上下文截止，并将剩余预算透传给指数、市场统计、板块和概念各自的 provider loop，超时后保留有效结果并继续策略报告
- [修复] API 个股报告的可选基本面与市场结构共享 45 秒编排截止，基本面最多使用 30 秒，所属板块与概念排行按剩余时间进入统一 provider loop 且单来源最多等待 10 秒，显式上层预算可钳制全局 provider override，避免批处理环境的放大超时导致任务卡死
- [修复] Web 个股报告不再把已有联合策略结论显示为“未单独评估”，并逐策略展示 Skill 声明的依赖输入、实际数据状态与联合/独立评估方式
