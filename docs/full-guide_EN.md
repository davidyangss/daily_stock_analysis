...
| `SERPAPI_TIMEOUT_SECONDS` | Timeout in seconds for the primary SerpAPI search request; defaults to `20`, valid range `1-300` | Optional |
| `SERPAPI_HARD_DEADLINE_SECONDS` | Maximum worker wait when the SerpAPI SDK stalls; defaults to `20`, valid range `1-300` | Optional |
...
| `AGENT_SKILL_AGENT_TIMEOUT_S` | Independent cap for one ordinary strategy Specialist; the effective cap is the lower of this value and the remaining orchestrator budget, and `0` disables the independent cap | `120` | No |
| `AGENT_STOCK_INFO_TIMEOUT_SECONDS` | Caller budget for ordinary Agent `get_stock_info`; does not change Trader Analysis timeouts or shared provider defaults | `60` | No |
| `AGENT_STOCK_INFO_PROVIDER_TIMEOUT_SECONDS` | Per-provider slice inside `get_stock_info` for valuation fallback, board membership, and name resolution | `8` | No |
...
| `SERPAPI_TIMEOUT_SECONDS` | Timeout in seconds for the primary SerpAPI search request; defaults to `20`, valid range `1-300` | Optional |
| `SERPAPI_HARD_DEADLINE_SECONDS` | Maximum worker wait when the SerpAPI SDK stalls; defaults to `20`, with the stricter request/deadline bound applied | Optional |
...
| `DAILY_MARKET_CONTEXT_TIMEOUT_SECONDS` | Overall stock-analysis wait deadline for live daily market context, in seconds | `120` |
| `MARKET_CONTEXT_PROVIDER_TIMEOUT_SECONDS` | Per-provider budget inside daily market context generation, in seconds | `10` |
| `MARKET_CONTEXT_STAGE_TIMEOUT_SECONDS` | Per-stage budget inside daily market context generation, in seconds | `12` |
| `MARKET_CONTEXT_OVERVIEW_TIMEOUT_SECONDS` | Market-overview budget inside daily market context generation, in seconds | `48` |
| `STOCK_OPTIONAL_PROVIDER_TIMEOUT_SECONDS` | Per-provider budget for optional evidence in ordinary API stock analysis | `10` |
| `STOCK_OPTIONAL_EVIDENCE_TIMEOUT_SECONDS` | Overall deadline for optional evidence in ordinary API stock analysis | `90` |
| `AGENT_SKILL_AGENT_TIMEOUT_S` | Independent cap for one ordinary strategy Specialist; timeout becomes diagnostics and final decision continues | `120` |
| `AGENT_STOCK_INFO_TIMEOUT_SECONDS` | Overall ordinary-Agent stock-info tool budget | `60` |
| `AGENT_STOCK_INFO_PROVIDER_TIMEOUT_SECONDS` | Per-provider stock-info tool budget | `8` |
...
### Daily Market Context And Strategy Evidence

Daily market context is optional enrichment, not a hard prerequisite for completing an ordinary stock report. Stock analysis waits at most `DAILY_MARKET_CONTEXT_TIMEOUT_SECONDS=120` seconds, while internal provider, stage, and overview work remains bounded by the `10` / `12` / `48` second defaults. Optional fundamental and market-structure evidence in ordinary API analysis uses `STOCK_OPTIONAL_PROVIDER_TIMEOUT_SECONDS=10` and `STOCK_OPTIONAL_EVIDENCE_TIMEOUT_SECONDS=90`. When a deadline expires, already validated data is retained, missing parts are marked as degraded, and report generation continues.

Strategy YAML `required_tools` are hard data dependencies. `dashboard.strategy_data_evidence` uses `verification_scope=required_inputs`: `verified` means the declared inputs passed admission, not that the strategy's trading conditions were met. `limited` covers fallback/partial/estimated/stale inputs, while missing, fetch-failed, unsupported, or uncalled requirements are `insufficient` and cannot vote in synthesis.

When selected strategies exist, `standard` and `full` run Technical → Intel (plus Risk in `full`) → isolated per-strategy Specialists → deterministic `StrategyEngine` synthesis → Decision. A Specialist may use only its own tool results and explicitly prefetched Pipeline inputs, never another Specialist's evidence. One Specialist failure is non-critical and does not prevent the report from completing. `quick` keeps its fast path; any compatible joint evaluation must still provide a separate structured signal, confidence, actual field references, met/missed conditions, and limitations for every strategy. Web and complete Markdown reports render each met/missed condition as its own status-table row. Data-limit rows combine the generic unavailable/degraded state with the matching tool's `metric_details/missing_fields`, listing the exact metrics still needed plus current sources or failure reasons. Missing data is disclosed rather than inferred or fabricated.
...
- **Strategy selection** - The Home page supports explicitly selecting analysis strategy skills; reports show the selected strategies, overall/per-strategy decisions, and the actual analysis inputs. When `skills` is omitted, analysis uses the server default strategy so legacy clients keep existing behavior
...
> Note: The Web Home page exposes an explicit strategy selector. When users do not pick one, `skills` is not sent and legacy behavior is preserved; when selected, it is passed through to this endpoint and persisted in task status, history snapshots, and `report.details.strategy_data_evidence`. In `standard` / `full`, selected strategies are evaluated by isolated Specialists before deterministic synthesis and Decision. The report's “Strategy analysis details” block shows each strategy's required inputs, actual evidence status, decision, conditions, and limitations. `quick` or compatibility joint evaluation must still return a separate structured assessment for every strategy; an overall conclusion is never copied into a per-strategy result.
...
> Note: `GET /api/v1/history` list summaries can be paginated by `stock_code` for same-stock history and include optional trend, summary, model, and analysis-time price/change fields. `strategy_names` contains the display names of the strategies actually used (for example, Emotion Cycle or Volume Breakout); it never falls back to an operation conclusion, and older rows without strategy snapshots return an empty array. `created_at` and `/api/v1/history/stocks` `last_analysis_time` are ISO 8601 timestamps with the server timezone offset; date filters are still interpreted as server-local dates. The Web report page's "History Trend" drawer reuses this endpoint.
...
- A local daily-bar window is eligible only when it covers the latest completed trading session, has the required row count, contains no future/incomplete bars, provides numeric OHLCV values, and uses one compatible adjustment basis throughout. Rejected local windows record a reason and continue through the existing remote fallback chain; adjustment series are never silently mixed.
...
- Stock-news lookup reuses local `news_intel` first only when the exact canonical symbol, publication window, title/snippet/publisher, HTTP(S) URL, relevance admission, and at least one direct-stock match all pass. Missing dates, future/out-of-window content, invalid sources, or sector-only context continue to remote fallback. If neither local nor remote evidence exists, the result explicitly states that no news is available and does not fabricate a summary.
