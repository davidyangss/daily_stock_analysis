...
> Note: The Web Home page exposes an explicit strategy selector. When users do not pick one, `skills` is not sent and legacy behavior is preserved; when selected, it is passed through to this endpoint and persisted in task status, history snapshots, and `report.details.strategy_data_evidence`. The report's “Strategy analysis details” block shows selected strategies, the overall decision, available per-strategy decisions, and actual tool/prefetched inputs with their data status. In single-Agent or other non-Specialist execution, a strategy is explicitly marked as not separately evaluated instead of copying the overall decision into a per-strategy result.
