...
| `ENABLE_CHIP_DISTRIBUTION` | 启用筹码分布分析。系统优先使用东财成品筹码数据；东财异常、超时、返回空数据或核心字段无效时，依次使用新浪、腾讯不复权日线与换手率在本地计算同口径指标。GitHub Actions 用户需在 Repository Variables 中设置 `ENABLE_CHIP_DISTRIBUTION=true` 方可启用；workflow 默认关闭。 | `true` | 可选 |
...
> 行为说明：
> - A 股筹码链路按东财成品数据、新浪日线本地计算、腾讯日线本地计算的顺序降级；本地计算结果的来源分别标记为 `akshare_sina_calculated` 与 `akshare_tencent_calculated`。腾讯日线当前不支持北交所，北交所仅尝试东财和新浪。所有渠道失败时筹码保持缺失，不阻断主分析流程。
...
> - `FUNDAMENTAL_STAGE_TIMEOUT_SECONDS` 是估值、财务摘要、主力资金、龙虎榜和板块数据共享的总预算；`FUNDAMENTAL_FETCH_TIMEOUT_SECONDS` 是单个能力调用的上限。若希望慢速资金流请求不挤占后续能力，可例如设置总预算 `45` 秒、单能力上限 `15` 秒；总预算不应小于单能力上限。提高这些值会直接增加最坏分析耗时。
...
> 说明：`GET /api/v1/history` 的列表摘要可按 `stock_code` 分页查询同一股票历史，并返回趋势判断、分析摘要、模型名与分析时价格/涨跌幅等可选字段；`strategy_names` 返回本次实际采用的策略名称数组（例如“情绪周期”“放量突破”），不会回退为操作建议，旧记录缺少策略快照时返回空数组。`created_at` 与 `/api/v1/history/stocks` 的 `last_analysis_time` 使用带服务器时区偏移的 ISO 8601 时间戳；日期筛选仍按服务器本地日期解释。Web 报告页的“历史趋势”抽屉复用该接口加载同股历史。
...
> 说明：问股首轮未携带 `stock_code` 时，会在本地股票索引中识别消息中出现的中文股票全称、短名和别名。唯一匹配会自动建立当前标的上下文；多个匹配不会由模型猜测市场，服务端会直接返回候选列表，Web 可点击候选继续分析，也可改为手动输入股票代码。未上市公司或索引中没有的名称不会被映射为产业链股票，仍按普通问答处理。
